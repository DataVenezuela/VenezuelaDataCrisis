...
└─────────────────────────────┘     └──────────────────────┘
      ↓
Staging exporter → upsert batch /rest/v1/aportes → aportes (Supabase, directo)
...
hace red real: el destino staging (`/rest/v1/aportes` en Supabase) se
intercepta con `httpx.BaseTransport` inyectado en el `StagingExporter`.
...
## Capa 4 — Staging exporter (Issue #81, #187) y watermark por fuente (Issue #57)
...
upsert masivo (batch de `_BATCH_SIZE=100`) directo a `/rest/v1/aportes` en
Supabase (PostgREST). Hasta #187 el destino era `dataVenezuela` (Vercel) via
`POST /api/aportes` individual — Vercel actuaba como proxy HTTP fino entre el
pipeline y Supabase; con fuentes de ~108k registros esto significaba 108k
invocaciones de function (cold starts, 500s, 504s). Ver `AGENTS.md` para el
contexto completo del porqué.
...
- Construir el payload del aporte usando los contratos de
  ...
  Keys en snake_case, alineadas 1:1 con las columnas de la tabla `aportes`.
  A diferencia del contrato Zod que exigía **omitir** una clave opcional
  ausente (nunca `null`), PostgREST exige que todas las filas de un mismo
  batch de upsert tengan las mismas keys: los opcionales se mandan siempre
  presentes, con `null` cuando faltan.
- `external_id` es determinista (fingerprint v1 para Event/AcopioCenter,
  `deterministic_id` para Person) y es la columna `UNIQUE` sobre la que
  PostgREST hace el upsert (`ON CONFLICT (external_id) DO UPDATE`), así que
  re-correr la misma fuente no duplica (idempotencia).
- Clasificar la respuesta del batch: 200/201/204 → todo el batch enviado,
  cualquier otro status o error de red → todo el batch cuenta como error
  acumulado (nunca relanza, resiliencia por batch). El upsert es atómico por
  request: PostgREST inserta el array completo en una sola transacción, así
  que no hay éxito parcial fila-por-fila dentro de un mismo batch como sí
  había con el POST individual anterior.
- No hay forma barata de distinguir insert de update en la respuesta
  (`return=minimal` no trae el body de vuelta), así que `ExportResult.duplicates`
  queda siempre en `0` — es una regresión de observabilidad aceptada a cambio
  de evitar 108k requests individuales.
- Avanzar el watermark de la fuente (`POST /rest/v1/source_watermarks`, body
  `{"slug": "...", "watermark_at": "<ISO>"}`, `Prefer: resolution=merge-duplicates`)
  a `max(fetched_at)` menos un margen de seguridad
  (`_WATERMARK_SAFETY_MARGIN`, ver más abajo) solo cuando todos los batches de
  esa fuente terminaron en 200/201/204. Si cualquiera falla, el watermark no
  cambia.

Auth con Supabase: headers `apikey` y `Authorization: Bearer <service_role_key>`
(contrato estándar de PostgREST), mismo par de headers en `/rest/v1/aportes`
y en `/rest/v1/source_watermarks`. La `service_role_key` bypasea RLS — nunca
debe loguearse ni usarse fuera de este entorno server-side (GitHub Actions).
...
en cada corrida. La idempotencia por `external_id` (upsert en Supabase) absorbe
...
`source.id` viaja como valor del filtro `slug=eq.{id}` en el query string
(PostgREST), no como segmento de path; `validate_sources_config` igual exige
que sea único entre fuentes y que solo contenga `[a-zA-Z0-9_-]` (buena
práctica general, aunque ya no haya riesgo de path injection).
...
Modo dry-run silencioso: si falta cualquiera de `SUPABASE_URL` o
`SUPABASE_SERVICE_ROLE_KEY`, el exporter queda deshabilitado, no abre cliente
...
## 10. Staging (upsert batch a /rest/v1/aportes)

El export a JSONL en disco fue eliminado en #81. El destino final es la tabla
`aportes` en Supabase, vía upsert masivo directo a `/rest/v1/aportes` (ver
"Capa 4 — Staging exporter"). Cada aporte es una fila con `external_id`
determinista para idempotencia.

Ejemplo de payload (un batch = array de hasta 100 objetos como este):
...
1. UTF-8, JSON válido.
2. Un upsert por batch de hasta 100 registros (no un POST por registro).
3. `external_id` determinista (idempotencia por upsert).
4. Campos requeridos presentes.
5. Campos opcionales desconocidos como `null` (nunca omitidos: PostgREST
   exige las mismas keys en todas las filas de un mismo batch).
...
| Staging exporter (upsert batch `/rest/v1/aportes`) | ✅ Issue #81, #187 |
...
## Estado operacional — verificado en producción (01 jul 2026)

Esta sección documenta hechos confirmados corriendo el pipeline en
producción, no diseño. Ver `AGENTS.md` para el contexto completo dirigido a
agentes.

**Confirmado funcionando:**
- `encuentralos_tecnosoft` end-to-end: fetch → parse → PII → normalización →
  upsert batch directo a `/rest/v1/aportes` → tabla `aportes` en Supabase
  (#187 — eliminó a Vercel/`dataVenezuela` del path de ingest; ya no hay
  proxy HTTP intermedio ni límite de function timeout).
...
**Resuelto por #187 — cuello de botella del POST individual:**
Antes de #187, `staging_exporter.export_source` mandaba un POST individual
por registro a `/api/aportes` (Vercel): con `encuentralos_tecnosoft` en
~98.830 registros, eso eran ~98.830 invocaciones de function (cold starts,
500s, 504s). Ahora los registros se agrupan en batches de 100 y se mandan
como upsert masivo directo a Supabase (`/rest/v1/aportes`), reduciendo esto a
~988 requests. La garantía de que el watermark solo avanza si **todos** los
batches de la fuente terminaron en 200/201/204 (ver "Capa 4 — Staging
exporter" arriba) se preserva.

**Brecha activa — `page_size` sigue sin ser configurable:**
...
eso son ~4.941 páginas de fetch. #187 resolvió el cuello de botella del
POST/upsert, pero el fetch paginado sigue sin poder configurar `page_size`
desde el YAML — sigue siendo trabajo pendiente, no confundir ambos fixes.

**Infraestructura — Supabase y Vercel se gestionan por separado:**
El pipeline de ingest (este repo) ya no depende de Vercel para nada: escribe
directo a Supabase. `dataVenezuela` (Vercel) sigue siendo la app pública que
lee de la misma base de Supabase para mostrar los datos consolidados — eso
no cambia con #187. Mover el proyecto Supabase a otra organización no
actualiza las env vars de Vercel automáticamente; si `dataVenezuela` devuelve
403 o no refleja cambios hechos directo en Supabase, verificar primero que
las env vars de Vercel (`SUPABASE_URL`, `PARTNER_API_SALT`) apunten al
proyecto correcto antes de asumir que el bug está en el pipeline.