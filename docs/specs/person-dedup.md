...
| Columna | Valor |
|---------|-------|
| `left_person` | UUID de la persona |
| `right_person` | UUID de la persona |
...
- `UNIQUE (left_person, right_person)` en `dedup_candidates`
- Si el par ya existe → `UPDATE score, reasons, priority` (no duplicar)
- Si es nuevo → `INSERT`
...
| `scrapers/dedup/blocking.py` | Adaptar: leer de `aportes.block_keys`, implementar blocking laxo por phonetic_hash |
