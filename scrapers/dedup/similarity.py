...
def similarity_score(
    ...
    """
    left_cedula: str | None = left.get("cedula_hmac") or None  # type: ignore[arg-type]
    right_cedula: str | None = right.get("cedula_hmac") or None  # type: ignore[arg-type]
    ...
    cedula_sc = 1.0 if left_cedula == right_cedula else 0.0
    location_sc = _location_score(
        left.get("last_known_location"),  # type: ignore[arg-type]
        right.get("last_known_location"),  # type: ignore[arg-type]
    )
    age_sc = _age_overlap_score(
        left.get("age_range"),  # type: ignore[arg-type]
        right.get("age_range"),  # type: ignore[arg-type]
    )
