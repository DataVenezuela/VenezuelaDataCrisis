...
def test_person_minimal_record_scores_low() -> None:
    person = Person(full_name="Ana Perez", fuente="grupo")

    assert confidence_score(person) < 0.4


def test_person_cedula_hmac_significantly_increases_score() -> None:
    without_cedula = Person(full_name="Ana Perez", fuente="grupo")
    with_cedula = Person(full_name="Ana Perez", cedula_hmac="abc123", fuente="grupo")
